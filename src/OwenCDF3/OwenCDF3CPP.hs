...
foreign import ccall unsafe "OwenCDF3" c_OwenCDF3 :: CInt -> CSize -> CDouble ->
...
owenCDF3cpp algo nu t1 t2 delta1 delta2 = do
  case delta1 == [] of
    True -> return V.empty
    False -> do
      ...
      V.unsafeWith delta1vec $
        \v1 -> V.unsafeWith delta2vec $
          \v2 -> withForeignPtr fptr $
            c_OwenCDF3 (fromIntegral algo) (fromIntegral nu) (realToFrac t1)
